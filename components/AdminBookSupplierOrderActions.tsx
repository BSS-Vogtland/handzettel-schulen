...
export default function AdminBookSupplierOrderActions({
  orderId,
  orderNumber,
  supplierPortalUrl,
  canSend,
  ...
  async function sendOrder() {
    ...
    try {
      const response = await fetch(
        `/api/admin/book-supplier/orders/${encodeURIComponent(
          orderId,
        )}/send`,
        {
          method: "POST",
      ...
      setMessage(
        payload.message ||
          `Der Auftrag ${orderNumber} wurde versendet.`,
      );
  ...
  async function copyPortalUrl() {
    try {
      ...
      setMessage(
        "Der Auftragslink wurde kopiert.",
      );
      setError(null);
    } catch {
      setError(
        "Der Auftragslink konnte nicht kopiert werden.",
      );
    }
  }
  ...
  return (
    <div className="grid gap-3">
      ...
      {!paymentConfirmed ? (
        <div className="rounded-2xl border border-[#F1D1A8] bg-[#FFF8EE] p-3 text-sm font-bold text-[#A75B28]">
          Der Auftrag kann erst versendet werden, wenn die
          manuelle Zahlungsprüfung bestätigt wurde.
        </div>
      ) : null}
