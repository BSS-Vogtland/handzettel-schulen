...
type CustomerOfferItemNoteFormProps = {
  token: string;
  itemId: string;
  productName: string;
...
async function readJsonSafely(response: Response): Promise<SaveNoteResponse | null> {
  const rawText = await response.text();

  try {
    return rawText ? (JSON.parse(rawText) as SaveNoteResponse) : null;
  } catch {
    return null;
  }
}

export default function CustomerOfferItemNoteForm({
  token,
  itemId,
  productName,
  initialNote = "",
  disabled = false,
}: CustomerOfferItemNoteFormProps) {
  ...
  async function handleSave() {
    ...
    if (note.length > MAX_NOTE_LENGTH) {
      setErrorMessage(
        `Dein Hinweis ist zu lang. Bitte maximal ${MAX_NOTE_LENGTH} Zeichen verwenden.`
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/offer/${encodeURIComponent(token)}/items/${encodeURIComponent(
          itemId
        )}/customer-note`,
        {
          method: "PATCH",
  ...
  if (disabled) {
    ...
    return (
      <div className="mt-4 rounded-2xl border border-[#D6E7EF] bg-[#F5FAFD] px-4 py-3 text-sm leading-6 text-[#12395F]">
  ...
  return (
    <div className="mt-4 rounded-2xl border border-[#D6E7EF] bg-[#F5FAFD] p-4">
      <div className="mb-3 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm font-semibold leading-6 text-[#52616F]">
            Hier kannst Du uns eine kurze Info zu „{productName}“ hinterlassen,
            z. B. Wunschfarbe, Ersatzhinweis oder etwas, worauf wir achten sollen.
          </p>
        </div>
      </div>
      ...
      <textarea
        ...
        rows={3}
        placeholder="Optional: z. B. bitte wenn möglich in Blau, sonst Alternative okay."
