...
const HOLD_SET_AT = "2026-08-25T04:52:26.375303+00:00";
const RPC_CONFIRMATION = "CONFIRM_RELEASE_AFTER_SALES_FULFILLMENT_HOLD";
const REASON =
  "Controlled After-Sales Phase A production test: normal fulfillment HOLD release";
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabaseServer.rpc(
    "release_after_sales_fulfillment_hold",
    {
      ...
      p_expected_case_status: "in_review",
      p_expected_case_revision: 4,
      p_expected_fulfillment_revision: 1,
      p_expected_hold_set_at: HOLD_SET_AT,
  ...
  if (error) {
    return NextResponse.json(
      { ok: false, code: "FULFILLMENT_HOLD_RELEASE_RPC_BLOCKED" },
  ...
  if (rows.length !== 1) {
    return NextResponse.json(
      { ok: false, code: "FULFILLMENT_HOLD_RELEASE_CAS_MISS" },
      { status: 409, headers: HEADERS },
    );
  }
  ...
  return NextResponse.json(
    {
      ok: true,
      code: "FULFILLMENT_HOLD_RELEASED",
