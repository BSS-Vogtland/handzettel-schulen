...
type CreateDiscountCampaignBody = {
  name?: string;
  ...
  try {
    ...
    if (error) {
      return NextResponse.json(
        {
          ok: false,
          error: "Rabattaktionen konnten nicht geladen werden.",
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Unbekannter Fehler beim Laden der Rabattaktionen.",
      },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    if (error) {
      return NextResponse.json(
        {
          ok: false,
          error: "Rabattaktion konnte nicht erstellt werden.",
          details: error.message,
        },
        { status: 500 }
      );
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Unbekannter Fehler beim Erstellen der Rabattaktion.",
      },
      { status: 500 }
    );
  }
