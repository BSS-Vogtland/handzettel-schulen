...
import { NATIVE_EXTERNAL_RESULT_RECOVERY_CONFIRMATION } from "@/app/lib/lexware/lexwareNativeExternalResultRecoveryCore";
import { recoverNativeLexwareExternalResult } from "@/app/lib/lexware/lexwareNativeExternalResultRecoveryService";
...
export async function POST(request: Request, { params }: { params: Promise<{ invoiceId: string }> }) {
  ...
  let body: unknown;
  try { body = await readLimitedJsonBody(request, 512); }
  catch (error) {
  ...
  if (!hasExactConfirmation(body, NATIVE_EXTERNAL_RESULT_RECOVERY_CONFIRMATION)) {
    return NextResponse.json({ ok: false, code: "CONFIRMATION_INVALID" }, { status: 400, headers: HEADERS });
  }
  const result = await recoverNativeLexwareExternalResult(invoiceId);
  return NextResponse.json({ ok: result.ok, code: result.code, recovered: result.recovered }, { status: result.status, headers: HEADERS });
}
