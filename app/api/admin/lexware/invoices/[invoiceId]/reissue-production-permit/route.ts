...
import { reissueLexwareProductionWritePermit } from "@/app/lib/lexware/lexwareProductionWritePermitService";
import { LEXWARE_REISSUE_PERMIT_CONFIRMATION } from "@/app/lib/lexware/lexwareProductionWritePermitCore";
...
export async function POST(request: Request, { params }: { params: Promise<{ invoiceId: string }> }) {
  ...
  if (!hasExactConfirmation(body, LEXWARE_REISSUE_PERMIT_CONFIRMATION)) return NextResponse.json({ ok: false, code: "CONFIRMATION_INVALID" }, { status: 400, headers: HEADERS });
  try { return NextResponse.json({ ok: true, ...(await reissueLexwareProductionWritePermit(invoiceId)), expiresInMinutes: 30, jobMutationsPerformed: 0, claimPerformed: false, lexwareRequestsPerformed: 0, mailOperationsPerformed: 0 }, { headers: HEADERS }); }
  catch { return NextResponse.json({ ok: false, code: "PERMIT_REISSUE_BLOCKED" }, { status: 409, headers: HEADERS }); }
}
