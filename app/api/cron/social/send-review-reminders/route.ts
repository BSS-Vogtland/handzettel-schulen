...
function getOpenReviewPosts(event: ReminderEventRow) {
  ...
  return posts.filter((post) => post && post.id && post.review_url);
}

function buildSubject(event: ReminderEventRow) {
  const projectName = getProjectName(event);
  const count = event.open_review_count || getOpenReviewPosts(event).length;

  if (count === 1) {
    return `${projectName}: 1 Social-Beitrag wartet auf Review`;
  }

  return `${projectName}: ${count} Social-Beiträge warten auf Review`;
}
...
function buildTextEmail(event: ReminderEventRow) {
  ...
  const lines = [
    greeting,
    "",
    `für ${projectName} gibt es SocialPilot-Beiträge, die geprüft und freigegeben werden sollten.`,
    "",
    `Reminder: ${event.reminder_date_local} um ${event.reminder_time_local} (${event.timezone})`,
    `Offene Reviews: ${event.open_review_count}`,
    "",
  ];

  if (openReviewPosts.length > 0) {
    lines.push("Offene Beiträge:");

    for (const post of openReviewPosts) {
      lines.push("");
      lines.push(`- ${post.topic || "Ohne Titel"}`);

      if (post.publish_date_local) {
        lines.push(
          `  Veröffentlichung geplant: ${
            post.publish_weekday_local || ""
          } ${post.publish_date_local}`.trim()
        );
      }
    ...
    lines.push(
      "Es wurden keine konkreten Review-Links im Reminder-Payload gefunden."
    );
  }

  lines.push("");
  lines.push("Bitte prüfe die Beiträge und gib sie nur frei, wenn Inhalt, Bildbezug, CTA und Plattform-Fit passen.");
  lines.push("");
...
function buildHtmlEmail(event: ReminderEventRow) {
    ...
    openReviewPosts.length > 0
      ? openReviewPosts
          .map((post) => {
            ...
            const publishInfo = post.publish_date_local
              ? `<p style="margin:6px 0 0;color:#64748b;font-size:14px;">Veröffentlichung geplant: ${escapeHtml(
                  `${post.publish_weekday_local || ""} ${
            ...
            return `
              <div style="border:1px solid #e2e8f0;border-radius:14px;padding:16px;margin:14px 0;background:#ffffff;">
                ...
                <p style="margin:14px 0 0;">
                  <a href="${escapeHtml(
                    reviewUrl
                  )}" style="display:inline-block;background:#0f172a;color:#ffffff;text-decoration:none;padding:10px 14px;border-radius:10px;font-weight:700;">
                    Review öffnen
                  </a>
                  ${
                    postingUrl
                      ? `<a href="${escapeHtml(
                          postingUrl
                        )}" style="display:inline-block;margin-left:8px;color:#0f172a;text-decoration:underline;font-weight:600;">Posting ansehen</a>`
                      : ""
                  }
                </p>
              </div>
          ...
          .join("")
      : `
        <div style="border:1px solid #fecaca;border-radius:14px;padding:16px;margin:14px 0;background:#fef2f2;color:#991b1b;">
          Es wurden keine konkreten Review-Links im Reminder-Payload gefunden.
        </div>
      `;
  ...
  return `
    ...
    <html lang="de">
      <head>
        <meta charset="utf-8" />
        <title>${projectName} Review-Erinnerung</title>
      </head>
        ...
        <div style="max-width:720px;margin:0 auto;padding:28px 18px;">
          <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:18px;padding:24px;">
            ...
            <h1 style="margin:0 0 12px;font-size:24px;line-height:1.25;color:#0f172a;">
              SocialPilot Review-Erinnerung
            </h1>

            <p style="margin:0 0 14px;font-size:16px;line-height:1.6;color:#334155;">
              Für <strong>${projectName}</strong> gibt es Social-Media-Beiträge, die geprüft und freigegeben werden sollten.
            </p>
            ...
            <div style="background:#f1f5f9;border-radius:14px;padding:14px;margin:18px 0;color:#334155;font-size:14px;">
              ...
              <div><strong>Offene Reviews:</strong> ${event.open_review_count}</div>
              <div><strong>Bereits freigegeben:</strong> ${event.approved_count}</div>
              <div><strong>Bereits veröffentlicht:</strong> ${event.published_count}</div>
            </div>

            ${postsHtml}

            <p style="margin:22px 0 0;font-size:15px;line-height:1.6;color:#475569;">
              Bitte prüfe die Beiträge und gib sie nur frei, wenn Inhalt, Bildbezug, CTA und Plattform-Fit passen.
            </p>
  ...
  try {
    ...
    if (pendingEvents.length === 0) {
      return NextResponse.json({
        ...
        message: "Keine pending Review-Reminder-Events zum Versenden gefunden.",
        summary: {
      ...
      try {
        ...
        if (dryRun) {
          results.push({
            ...
            status: "dry_run",
            message: "Dry-Run: Mail wurde nicht versendet und Event nicht verändert.",
          });
        ...
        results.push({
          ...
          recipient_email: event.recipient_email,
          status: "sent",
          message: "Review-Reminder-Mail wurde versendet.",
        });
      } catch (eventError) {
        const message =
    ...
    return NextResponse.json({
      ...
      message: dryRun
        ? "Dry-Run abgeschlossen. Es wurden keine Mails versendet und keine Events verändert."
        : "Pending Review-Reminder-Events wurden verarbeitet.",
      summary: {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unbekannter Fehler beim Versenden der Review-Reminder.";
