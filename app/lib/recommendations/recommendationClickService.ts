...
function excludedItem(row: Record<string, unknown>) {
  return EXCLUDED_ITEM_STATUSES.has(status(row.status))
...
export async function createRecommendationClick(input: {
  ...
  const [partnerResult, categoryResult, ruleResult, linkResult, requestResult, itemResult, offerResult, childResult] =
    await Promise.all([
      supabase
        .from("recommendation_partners")
        .select("id,project_key,partner_code,name,slug,target_url,active,attribution_days")
      ...
      supabase
        ...
        .select("id,request_id,child_id,status,admin_resolution_status")
  ...
  if (
    ...
    || excludedItem(item)
    || offerResult.data
    || (context.childId && (child.request_id !== context.requestId || child.is_active !== true))
  ) {
    throw new RecommendationClickServiceError();
  }

  const targetUrl = validateRecommendationHttpUrl(partner.target_url);
  if (!targetUrl) throw new RecommendationClickServiceError();
  ...
  const attributionExpiresAt = new Date(
    clickedAt.getTime() + attributionDays * 24 * 60 * 60 * 1000,
  );
  const clickToken = randomBytes(24).toString("base64url");
  const userAgent = nullableString(input.userAgent)?.slice(0, 512) ?? null;
  const { error: insertError } = await supabase.from("recommendation_clicks").insert({
  ...
  if (insertError) throw new RecommendationClickServiceError();

  const redirectUrl = addRecommendationClickTokenToUrl(targetUrl, clickToken);
  if (!redirectUrl) throw new RecommendationClickServiceError();
  ...
  return {
    ...
    attributionMaxAgeSeconds: attributionDays * 24 * 60 * 60,
...
function normalizeAdminRow(value: unknown): RecommendationClickAdminRow {
  ...
  return {
    ...
    attributionExpiresAt: requiredString(row.attribution_expires_at),
...
export async function listRecommendationClicks(
  ...
  const projectKey = recommendationProjectKey(
    options.projectKey ?? DEFAULT_RECOMMENDATION_PROJECT_KEY,
  );
  const search = recommendationSearch(options.search);
  const page = Math.max(1, Math.trunc(options.page ?? 1));
  const limit = Math.min(100, Math.max(1, Math.trunc(options.limit ?? 50)));
  if (options.partnerId && !isRecommendationUuid(options.partnerId)) {
    throw new RecommendationClickServiceError("Der Partnerfilter ist ungültig.");
  }
  if (options.categoryId && !isRecommendationUuid(options.categoryId)) {
    throw new RecommendationClickServiceError("Der Kategoriefilter ist ungültig.");
  }
  const supabase = getRecommendationAdminClient();
  let query = supabase
    .from("recommendation_clicks")
    .select(
      "id,click_token,partner_id,category_id,rule_id,request_id,child_id,request_item_id,partner_code_snapshot,partner_name_snapshot,category_name_snapshot,matched_term,clicked_at,attribution_expires_at,referrer_origin,is_probable_bot",
      { count: "exact" },
    )
    .eq("project_key", projectKey);
  if (options.partnerId) query = query.eq("partner_id", options.partnerId);
  if (options.categoryId) query = query.eq("category_id", options.categoryId);
  if (options.bot === "bot") query = query.eq("is_probable_bot", true);
  if (options.bot === "human") query = query.eq("is_probable_bot", false);
  const fromDate = dateBoundary(options.dateFrom, false);
  const toDate = dateBoundary(options.dateTo, true);
  if (fromDate) query = query.gte("clicked_at", fromDate);
  if (toDate) query = query.lt("clicked_at", toDate);
  if (search) {
    query = query.or(
      `partner_name_snapshot.ilike.%${search}%,partner_code_snapshot.ilike.%${search}%,category_name_snapshot.ilike.%${search}%,matched_term.ilike.%${search}%,click_token.ilike.%${search}%`,
    );
  }
  ...
  if (error) {
    throw new RecommendationClickServiceError("Klickdaten konnten nicht geladen werden.");
  }
  return {
