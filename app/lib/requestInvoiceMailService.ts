...
export type RequestInvoiceMailPayload = {
  ok: boolean;
  message: string;
  invoiceId?: string;
...
export type RequestInvoiceMailResult = {
  data: RequestInvoiceMailPayload;
  status: number;
};
...
export async function sendRequestInvoiceMail(input: {
  requestId: string;
}): Promise<RequestInvoiceMailResult> {
  try {
    ...
    const pdf = await generateRequestInvoicePdf({
      requestId,
    });
