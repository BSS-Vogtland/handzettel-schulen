export const LEXWARE_PRODUCTION_FINALIZE_CONFIRMATION =
  "LEXWARE_PRODUCTION_FINALIZE_EXACTLY_ONCE_V1" as const;
...
export type LexwareRuntimeConfiguration = {
  activeMode: string | null;
  integrationEnabled: boolean;
};
...
export class LexwareProductionInvoiceWriteError extends Error {
  ...
  constructor(
    ...
    super(message);
    this.name = "LexwareProductionInvoiceWriteError";
    this.code = code;
...
export type LexwareProductionWriteDependencies = {
  ...
  runtimeConfiguration: LexwareRuntimeConfiguration;
  connectionConfiguration: LexwareConnectionConfiguration;
  evaluateGates(input: LexwareProductionGateInput): LexwareProductionGateResult;
  currentTime: () => string;
...
export async function executeLexwareProductionInvoiceWrite(
  input: CreateLexwareProductionInvoiceInput,
  dependencies: LexwareProductionWriteDependencies,
  ...
  const gateResult = dependencies.evaluateGates(input.gates);
  ...
  try {
    ...
    // Classified below as an ambiguous response after a successful HTTP status.
  }
  if (!response.ok) {
