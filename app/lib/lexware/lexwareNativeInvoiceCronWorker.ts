...
import {
  ...
  isNativeInvoiceCronTargetFirst,
...
export async function loadNativeInvoiceCronCandidates(): Promise<NativeInvoiceCronCandidate[]> {
...
export async function isNativeInvoiceCronTargetReady(invoiceId: string): Promise<boolean> {
  return isNativeInvoiceCronTargetFirst(await loadNativeInvoiceCronCandidates(), invoiceId, Date.now());
}
...
export async function processNextNativeLexwareInvoice(): Promise<NativeInvoiceCronWorkerResult> {
  return runNativeInvoiceCronWorker({
    now: Date.now,
    loadCandidates: loadNativeInvoiceCronCandidates,
