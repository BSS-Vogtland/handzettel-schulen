...
function getCleanCustomerOfferItemNote(item: { notes?: string | null }) {
  ...
  if (
    ...
    return `Automatisch vorausgewÃ¤hlt, da der Produkttreffer ${score} % Ãœbereinstimmung erreicht hat.`;
  }
...
function getCustomerVisibleOpenItemCount(
  ...
  return items.filter((item) => {
    ...
    return !isRequestItemResolvedForWorkflow(item, selected.length > 0);
  }).length;
}
                        ...
                        >
                          ...
                          {itemMatches.length > 0 ? (
                            ...
                            <p className="rounded-2xl border border-dashed border-[#F1D1A8] bg-[#FFF8EE] p-3 text-sm font-semibold text-[#A75B28]">
                              FÃ¼r diese Position gibt es noch keinen sicheren Produktvorschlag. Du kannst unten selbst suchen oder die Position vom Team prÃ¼fen lassen.
                            </p>
                          )}
...
if (productIds.length > 0) {
  ...
  const workflowBlockingState = getRequestBlockingState(
    items,
    selectedOfferItems,
    request.checkout_override_enabled === true
  );

  const openChoiceItems = workflowBlockingState.rawBlockingItems.filter((item) => {
    const itemMatches = matchesByItem.get(item.id) || [];
    const selectableMatches = itemMatches.filter(isSelectableOpenMatch);

    return selectableMatches.length > 0;
  });

  const openChoiceItemIds = new Set(openChoiceItems.map((item) => item.id));

  const manualReviewItems = workflowBlockingState.rawBlockingItems.filter(
    (item) => !openChoiceItemIds.has(item.id)
  );
  ...
  const handledItemCount =
    items.length - workflowBlockingState.rawBlockingCount;

  const hasOpenCustomerBlockingItems =
    workflowBlockingState.effectiveBlockingCount > 0;

  const openDecisionItemCount = hasOpenCustomerBlockingItems
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-3 max-w-3xl text-sm leading-6 text-[#52616F] sm:text-base sm:leading-7">
                    {isConfirmed
                      ? "Dein Paketwunsch wurde an Handzettel-Schulen.de Ã¼bermittelt. Wir prÃ¼fen."
